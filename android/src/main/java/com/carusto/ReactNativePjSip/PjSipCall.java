...
public class PjSipCall extends Call {
	...
	public void updateRX(float rxLevel) throws Exception {
		doUpdateRX(rxLevel);
	}
	...
	private void doUpdateRX(float rxLevel) throws Exception {
		CallInfo info;
		try {
			info = getInfo();
		} catch (Exception exc) {
		...
		for (int i = 0; i < info.getMedia().size(); i++) {
			...
			if (mediaInfo.getType() == pjmedia_type.PJMEDIA_TYPE_AUDIO
				...
				try {
					...
					audioMedia.adjustRxLevel((float) rxLevel); //Received from the others, transmitted by yourself.
				} catch (Exception exc) {
					Log.e(TAG, "error", exc);
				}
			}
